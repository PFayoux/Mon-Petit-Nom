...
**Recherche** (Résultats) :
Le champ sous les onglets de statut filtre la liste affichée par correspondance de préfixe, insensible à la casse et aux accents (« e » trouve aussi « Émile »). Elle ne filtre qu'à l'intérieur de l'onglet de statut actif, en resserrant ce que montrent déjà les onglets de genre et Moi/Partenaire — jamais en les contournant. Les compteurs entre parenthèses sur les onglets de statut restent calculés sur l'ensemble non filtré : la recherche ne change que la liste affichée, pas ces chiffres. La requête reste active en changeant d'onglet (statut, genre, vue) ; elle n'est réinitialisée qu'en appuyant sur le bouton d'effacement.
_Avoid_: Filtre (seul — la recherche est un filtre texte, à distinguer des filtres genre/statut déjà nommés par ailleurs)
