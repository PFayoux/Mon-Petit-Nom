...
**Métadonnées d'origine** (`name.originLanguage` / `originReligion` / `originCountry`) :
Informations d'étymologie extraites une fois par un script batch (Wiktionnaire + Claude Haiku, voir [ADR-0008](./docs/adr/0008-name-origin-metadata-via-llm-batch.md)), stockées sur chaque prénom du dataset statique. Purement informatives/affichage — jamais un critère de filtre (voir [ADR-0002](./docs/adr/0002-name-dataset-storage.md)). Nullables indépendamment : l'absence de mention explicite dans le texte source vaut `null`, jamais une valeur déduite/inventée.
_Avoid_: Signification (terme plus large que ce qu'on extrait réellement — on ne capture que langue/religion/pays, pas un sens global du prénom)
